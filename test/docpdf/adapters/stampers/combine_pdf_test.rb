...
if COMBINE_PDF_AVAILABLE && PRAWN_AVAILABLE
  require "docpdf/adapters/stampers/combine_pdf"

  class StamperCombinePdfTest < Minitest::Test
    def test_stamp_all_positions
      positions = [:center, :top, :bottom, :left, :right, :top_left, :top_right, :bottom_left, :bottom_right]
      positions.each do |pos|
        result = DocPDF::Adapters::Stampers::CombinePdf.stamp(sample_pdf, [stamp(position: pos)])
    ...
    def test_stamp_page_indices
      result = DocPDF::Adapters::Stampers::CombinePdf.stamp(multi_page_pdf, [stamp], page_indices: [0])
      assert valid_pdf?(result)
      assert_equal 3, pdf_page_count(result)
    end

    def test_stamp_preserves_page_count
      result = DocPDF::Adapters::Stampers::CombinePdf.stamp(multi_page_pdf, [stamp])
      assert_equal 3, pdf_page_count(result)
    end

    def test_stamp_produces_valid_pdf
      result = DocPDF::Adapters::Stampers::CombinePdf.stamp(sample_pdf, [stamp(opacity: 0.1)])
      assert valid_pdf?(result)
    end
    ...
    def test_text_stamp_all_positions
      positions = [:center, :top, :bottom, :left, :right, :top_left, :top_right, :bottom_left, :bottom_right]
      positions.each do |pos|
        result = DocPDF::Adapters::Stampers::CombinePdf.stamp(sample_pdf, [text_stamp(position: pos)])
        assert valid_pdf?(result), "Failed for position: #{pos}"
      end
    end
